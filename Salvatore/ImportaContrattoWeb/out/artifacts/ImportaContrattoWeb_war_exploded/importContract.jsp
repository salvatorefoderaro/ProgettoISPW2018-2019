...
<%
    ...
    if (localStartDate.until(localEndDate).getDays() <= 30){
        session.setAttribute("wrongDaysIntervalMin", "30");
    ...
    if (localStartDate.until(localEndDate).getDays() <= 180){
        session.setAttribute("wrongDaysIntervalMax", "180");
        String destination ="importContract.jsp";
        response.sendRedirect(response.encodeRedirectURL(destination));
        return;
    }
...
<div class="container .text-center" style="margin: 0px auto;">
    ...
    <% session.setAttribute("transactionError", null);
        ...
        if (session.getAttribute("wrongDaysIntervalMin") != null) {  %>


    <div class="alert alert-warning">
        <strong>Errore!</strong> L'intervallo minimo è di 30 giorni!
    </div>


    <% session.setAttribute("wrongDaysIntervalMin", null);
    }
