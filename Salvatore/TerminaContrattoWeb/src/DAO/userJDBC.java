...
public class userJDBC implements userDAO {
    ...
    public userSessionBean login(userSessionBean bean) throws SQLException, emptyResult {
        ...
        if (resultSet.next() == false){
            ...
            userSessionBean loggedUser = new userSessionBean(resultSet.getString("Nickname"), resultSet.getInt("ID"), resultSet.getString("tableName"), resultSet.getInt("paymentClaim"), "", null);
            resultSet.close();
            preparedStatement.close();
            return loggedUser;
        }
    ...
    public userSessionBean getLocatario(userSessionBean session) throws SQLException {
        ...
        while(resultSet.next()){
            tenant = new userSessionBean(resultSet.getString("tenantNickname"), resultSet.getInt("tenantID"), "tenant", resultSet.getInt("tenantPaymentClaimNumber"), "", null);
        }
        resultSet.close();
        preparedStatement.close();
