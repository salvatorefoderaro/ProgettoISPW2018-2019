...
public class ContractJDBC {
    ...
    public void createContract(ContractBean contractBean) throws SQLException, ClassNotFoundException, ConfigFileException, ConfigException, ContractPeriodException{
        ...
        try {
            ...
            String sql = "SELECT aptId, renterNickname, tenantNickname, aptToRentId, roomToRentId, bedToRentId, type, contractTypeId, startDate, endDate, price, deposit FROM ContractRequest WHERE id = " + contractBean.getContractRequestId().getId();
            ...
            if (!verifyAvaibility(conn, "aptToRentId", rs1.getInt("aptId"), rs1.getDate("startDate").toLocalDate(), rs1.getDate("endDate").toLocalDate())) {
                conn.rollback();
                throw new ContractPeriodException(); //TODO Cambiare eccezione
            }
            ...
            switch (RentableTypeEnum.valueOf(rs1.getString("type"))) {
                case APTTORENT:
                    ...
                    do {
                        if(!verifyAvaibility(conn, "roomToRentId", rs2.getInt("id"), rs1.getDate("startDate").toLocalDate(), rs1.getDate("endDate").toLocalDate())) {
                            conn.rollback();
                            throw new ContractPeriodException(); //TODO Cambiare eccezione
                        }
                        ...
                        do {
                            if(!verifyAvaibility(conn, "bedToRentId", rs3.getInt("id"), rs1.getDate("startDate").toLocalDate(), rs1.getDate("endDate").toLocalDate())) {
                                conn.rollback();
                                throw new ContractPeriodException(); //TODO Cambiare eccezione
                            }
                        } while (rs3.next());
                ...
                case ROOMTORENT:
                    ...
                    do {
                        if(!verifyAvaibility(conn, "bedToRentId", rs4.getInt("id"), rs1.getDate("startDate").toLocalDate(), rs1.getDate("endDate").toLocalDate())) {
                            conn.rollback();
                            throw new ContractPeriodException(); //TODO Cambiare eccezione
                        }
                    } while (rs4.next());
                ...
                case BEDTORENT:
                    ...
                    if(!verifyAvaibility(conn, "roomToRentId", rs5.getInt("id"), rs1.getDate("startDate").toLocalDate(), rs1.getDate("endDate").toLocalDate())) {
                        conn.rollback();
                        throw new ContractPeriodException(); //TODO Cambiare eccezione
                    }
                    break;
            ...
            String insertString = "INSERT INTO Contract (aptId, tenantNickname, "+ column +", type, contractTypeId, state,  creationDate, startDate, endDate, tenantName, tenantSurname, tenantCF, tenantDateOfBirth, tenantCityOfBirth, tenantAddress, renterName, renterSurname, renterCF,renterAddress, price, deposit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            String insertService = "INSERT INTO Contract_has_Service(contractId, serviceId) VALUES (" + contractId + ", ?)";

            preparedStatement2 = conn.prepareStatement(insertService);

            sql = "SELECT serviceId FROM ContractRequest_has_Service WHERE contractRequestId = " + contractBean.getContractRequestId().getId();
            ...
            sql = "UPDATE ContractRequest SET state = '" + RequestStateEnum.APPROVED.toString() + "'WHERE id = " + contractBean.getContractRequestId().getId();
    ...
    private boolean verifyAvaibility(Connection conn, String idType, int id, LocalDate startDate, LocalDate endDate) throws SQLException {
        ...
        try {
            ...
            String sql = "SELECT startDate, endDate FROM Contract WHERE " + idType + " = " + id + " AND (startDate <= DATE('" + endDate.toString() + "') " +
